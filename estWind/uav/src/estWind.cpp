...
estWind::estWind(TargetController *controller): UavStateMachine(controller), behaviourMode(BehaviourMode_t::Default), vrpnLost(false), controlMode_t(ControlMode_t::Default) {
    Uav* uav=GetUav();
...
void estWind::ComputeCustomTorques(Euler &torques)
{
    ...
    switch (control_selection->CurrentIndex())
    {
        ...
        default:
            ...
            Thread::Warn("estWind: default control law started. Check custom torque definition. \n");
            EnterFailSafeMode();
            break;
    }
}

void estWind::computeMyCtrl(Euler &torques)
{
...
float estWind::ComputeCustomThrust(void)
{
...
void estWind::StartCustomTorques(void)
{
    if (control_selection->CurrentIndex() == 0)
    {
        StartDefaultTorques();
        Start_task();
        Thread::Info("estWind: default control law started\n");
    }
    else
    {
        if(SetTorqueMode(TorqueMode_t::Custom) && SetThrustMode(ThrustMode_t::Custom) && control_selection->CurrentIndex() != 0)
        {
            ...
            myCtrl->Reset();
            Start_task();
            Thread::Info("estWind: custom control law started\n");
        }
        else
        {
            StopCustomTorques();
            Thread::Err("estWind: could not start custom control law\n");
        }
    }
    
}

void estWind::StopCustomTorques(void)
{
    StartDefaultTorques();
    controlMode_t = ControlMode_t::Default;
    Thread::Info("estWind: custom control law stopped\n");
}

void estWind::StartDefaultTorques(void)
{
    if (controlMode_t == ControlMode_t::Default)
    {
        Thread::Warn("estWind: already in default control law\n");
        return;
    }
    ...
    {
        Thread::Err("estWind: could not start default control law\n");
    }
}

const AhrsData *estWind::GetOrientation(void) const {
    //get yaw from vrpn
...
void estWind::AltitudeValues(float &z,float &dz) const{
    Vector3Df uav_pos,uav_vel;
...
AhrsData *estWind::GetReferenceOrientation(void) {
...
void estWind::PositionValues(Vector2Df &pos_error,Vector2Df &vel_error,float &yaw_ref) {
...
void estWind::SignalEvent(Event_t event) {
...
void estWind::ExtraSecurityCheck(void) {
...
void estWind::ExtraCheckPushButton(void) {
...
void estWind::ExtraCheckJoystick(void) {
...
void estWind::Start_task(void) {
    if( behaviourMode==BehaviourMode_t::Circle) {
        Thread::Warn("estWind: already in circle mode\n");
        return;
    }
    if (SetOrientationMode(OrientationMode_t::Custom)) {
        Thread::Info("estWind: start circle\n");
    } else {
        Thread::Warn("estWind: could not start circle\n");
        return;
    }
    ...
    if (task_selection->CurrentIndex() == 0) {
        ...
        Thread::Info("estWind: circle tracking\n");
    }
    else {
        Thread::Err("estWind: unknown task\n");
        return;
    }
}

void estWind::StopCircle(void) {
    if( behaviourMode!=BehaviourMode_t::Circle) {
        Thread::Warn("estWind: not in circle mode\n");
        return;
    }
    circle->FinishTraj();
    //GetJoystick()->Rumble(0x70);
    Thread::Info("estWind: finishing circle\n");
}

void estWind::VrpnPositionHold(void) {
    if( behaviourMode==BehaviourMode_t::PositionHold) {
        Thread::Warn("estWind: already in vrpn position hold mode\n");
        return;
    }
	Quaternion vrpnQuaternion;
    uavVrpn->GetQuaternion(vrpnQuaternion);
    ...
    Thread::Info("estWind: holding position\n");
}
